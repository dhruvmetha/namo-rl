#!/bin/bash
# F1+F3 fix — hop_1/benchmark_3, GPU 1, max_episode_steps=750
set -u
cd "$(dirname "$0")/.."
...
LOGFILE="screen_logs/f1f3_hop1_seed1.log"
echo "[$(date)] starting hop_1 run on $(hostname), GPU=$CUDA_VISIBLE_DEVICES, py=$PYBIN" | tee "$LOGFILE"
$PYBIN -u -m tdmpc_square_namo.train \
  ...
  device='cuda:0' \
  exp_name='vanilla_namo_hop1_seed1_tol_gtol05_tb100_wp10_eps750' \
  ...
  2>&1 | tee -a "$LOGFILE"
echo "[$(date)] hop_1 run exited" | tee -a "$LOGFILE"
